...
describe('FIX-02 countdown generation', () => {
  it('sends GO on the current open socket after countdown', async () => {
    ...
    assert.equal(h.sent[0].start_path, 'normal_countdown')
    assert.deepEqual(h.countdowns.filter(v => v != null), [3, 2, 1, 'GO'])
...
describe('PATCH-02 prepared resumed flag', () => {
  ...
  it('prepared resumed=false does not recapture before a new GO', async () => {
    ...
    assert.equal(h.capture.start, starts)
    assert.equal(h.session.goAcked, false)
    const go = h.sent.find(m => m.type === 'go')
    assert.ok(go)
    assert.equal(go.start_path, 'reconnect_reactivation')
...
describe('manual Start countdown vs reconnect skip', () => {
  it('reconnect during countdown restarts 3-2-1-GO and does not skip', async () => {
    const h = harness()
    h.session.handlePrepared({ live_session_id: 'sid', resumed: false })
    await h.session.requestStart()
    assert.equal(h.countdowns.filter(v => v === 3).length, 1)
    h.timers.advance(COUNTDOWN_STEP_MS)
    assert.equal(h.countdowns.at(-1), 2)
    h.session.handleDisconnect()
    h.sent.length = 0
    h.session.handlePrepared({ live_session_id: 'sid', resumed: true })
    assert.equal(h.session.phase, 'countdown')
    assert.equal(h.session.goAcked, false)
    assert.equal(h.countdowns.at(-1), 3)
    h.timers.advance(COUNTDOWN_STEP_MS * 4)
    const go = h.sent.find(m => m.type === 'go')
    assert.ok(go)
    assert.equal(go.start_path, 'normal_countdown')
    assert.equal(h.capture.start, 0)
    h.session.handleGoAck()
    assert.equal(h.capture.start, 1)
    assert.equal(h.session.startPath, 'normal_countdown')
  })

  it('reconnect after go_ack skips countdown and records reactivation', async () => {
    const h = harness()
    await startThroughCountdown(h)
    h.session.handleGoAck()
    h.sent.length = 0
